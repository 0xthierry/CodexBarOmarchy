...
const fetchGeminiApiSnapshot = async (
  ...
  if (
    ...
    try {
      credentials = await refreshGeminiAccessToken(host, resolvedSource.oauthPath, credentials);
    } catch (error) {
  ...
  const fetchQuotaSnapshot = async (
    ...
    if (quotaResponse.statusCode === 401) {
      ...
      try {
        refreshedCredentials = await refreshGeminiAccessToken(
          host,
